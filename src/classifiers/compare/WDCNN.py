...
class WDCNN(nn.Module):
    def __init__(self, kernel_size, numclass):
        super(WDCNN, self).__init__()
        self.conv1 = nn.Sequential(
            nn.Conv2d(1, 16, kernel_size =(1,64), stride = 8,#16
                               padding=(0, 64//2)),
            nn.BatchNorm2d(16),
            nn.ReLU(),
            nn.MaxPool2d((1,2)),
        ...
        self.conv2 = nn.Sequential(#32
            nn.Conv2d(16, 32, kernel_size =(1,kernel_size), stride = 1,
        ...
        self.conv3 = nn.Sequential(
            nn.Conv2d(32, 64, kernel_size =(1,kernel_size), stride = 1,
                      padding=(0, kernel_size // 2), bias=True),
            nn.BatchNorm2d(64),
            nn.ReLU(),
            nn.MaxPool2d((1, 2)),
        )
        self.conv4 = nn.Sequential(
            nn.Conv2d(64, 64, kernel_size =(1,kernel_size), stride = 1,
                               padding=(0,kernel_size//2), bias=True),
            nn.BatchNorm2d(64),
            nn.ReLU(),
            nn.MaxPool2d((1, 2)),
        ...
        # self.fc2 = nn.Linear(100,numclass)

        self.fc = nn.Sequential(
            nn.Linear(64*4,100),
            nn.ReLU(),
            nn.Linear(100,numclass),
            )
